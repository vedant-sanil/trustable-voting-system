...
public class SeriesReport implements Serializable
{
    ...
    public void printSummary(PrintStream stream)
    {
        ...
        double total = successful_tests.size() + failure_reports.size() + abandoned_tests.size();
        double score = total == 0 ? 0 : (Double.valueOf(successful_tests.size()) / total * 100);
        int expect_score = total == 0 ? 0 : 100;
        stream.println("score: " + String.format("%.2f", score) + "/" + expect_score);
    }
